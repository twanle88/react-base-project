...
export const removeUserSessionWhenExpired = () => {
  if (
    !publicRoutes.filter((item) => item.path === document.location.pathname).length // publicRoutes
    // && !unauthorizeRoutes.filter((item) => item.path === document.location.pathname).length // unauthorizeRoutes
  ) {
    window.localStorage.removeItem(USER_SESSION_KEY);
    window.location.href = `${window.location.origin}/app/sign-in`;
  }
};
